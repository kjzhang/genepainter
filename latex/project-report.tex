...
\subsection{Mutation}
We incorporate multiple levels of mutation to provide for the maximum level of adjustability. At the highest level, we have that each gene which is a stroke can be mutated via addition, editing, or deletion. In the case of addition which has an adjustable rate parameter, we append a completely random new stroke based on the same initial generation parameters to the DNA sequence. In the case of deletion, we can choose to delete one of the genes or strokes completely from the sequence. Finally, in the case of editing, we can choose to replace the gene with a completely new gene, i.e. addition and deletion combined.
However, we note our original mutation for editing may be too random and hence, we have a separate level of mutations. Instead of simply changing the entire stroke, we mutate specific attributes of the stroke, which should prove to be useful when trying to fine tune an image instead of attempting to completely start fresh. We structure the random generation by creating normal distributions centered around the current values for the locations, width, opacity, and color with an adjustable standard deviation. Essentially, instead of randomly guessing with no bias, we attempt to shift the stroke slightly in a random direction and see if we can find any improvements nearby. Additionally, in this case, we can also choose to remove or add points in the spline of the stroke.

\subsection{Crossovers}
DNA crossover was implemented in multiple ways. First, the naive method we chose to use was to pick two DNA strands and and uniformly pick random genes and/or strokes from both strands. Additionally, we pick one strand randomly over the other to determine the length of the strand. However, we expect such a method to yield a random data that does not significantly improve the result. Given that a group of strokes together are what most likely cause the fitness to do well as a whole, removing one of these from the group or merging two groups will not necessarily improve the fitness unless they are a large mutually exclusive set. The second approach we attempt is to swap out entire regions from the DNA strand, which is conceptually a single-pooint crossover, in an attempt to transfer entire groups of strokes that work well together.
...
\begin{figure*}[!htb]
\begin{center}
\includegraphics[width=2in]{../images/picasso128.png}\qquad
\includegraphics[width=2in]{../images/ML257.png}
\caption{Picasso and Mona Lisa, our evolution targets.}
\label{jpegfig}
\end{center}
\end{figure*}
...
The bulk of our implementation was done in Python, and requires the additional NumPy and matplotlib libraries for ND array manipulations and image rendering. Some initial investigatory work, such as the BTTC implementation, was done in MATLAB. The following sections detail the specifics of our implementation.
...
Intuitively, this would allow us to determine the general strokes necessary to produce the key features of the picture quickly. The remaining details which would normally take an exorbitant amount of time to be fine tuned would be done much quicker via a narrowed range of mutation as described above.

\subsection{Dumping DNA Outputs}
...
In comparison to our minimal supervision, the maximal supervision incorporates crossovers as specified above as well as a child retention policy. In addition, we attempt to factor in mutation on a gene level by replacing the stroke editing with a gene mutation.

Additionally, we have also tested strategies involving hill climbing. Starting with an initial set of strokes, we attempt to randomly mutate them until we find a local minimum. Upon finding the local minimum, we add a new stroke before returning to the mutation process. However, because we want these intial strokes to be the base and want to focus on mutating these newly added strokes, we scale the probability of mutating so that the newer strokes have a higher probability. In particular, we used

\[ P_k = P_c\frac{k^2}{\sum\limits_{i=0}^n i^2} \]
where $P_c$ is a constant and $n$ is the current number of strokes in the pool.

\section{Experiments and Results}
Below, we present the optimal results from running on our machines as well as on the AWS cluster. The simulations took approximately 8 hours each on a 4.4Ghz i-3570k machine. 

\begin{figure*}[!htb]
\begin{center}
\includegraphics[width=2in]{../TEMP/M-2.png}\qquad
\includegraphics[width=2in]{../TEMP/M-50.png}\qquad
\includegraphics[width=2in]{../TEMP/M-100.png}\\
\includegraphics[width=2in]{../TEMP/M-402.png}\qquad
\includegraphics[width=2in]{../TEMP/M-625.png}\qquad
\includegraphics[width=2in]{../TEMP/M-1100.png}\\
\includegraphics[width=2in]{../TEMP/M-1525.png}\qquad
\includegraphics[width=2in]{../TEMP/M-1850.png}\qquad
\includegraphics[width=2in]{../TEMP/M-2508.png}\\
\includegraphics[width=2in]{../TEMP/M-6602.png}\qquad
\includegraphics[width=2in]{../TEMP/M-15130.png}\qquad
\includegraphics[width=2in]{../TEMP/M-46057.png}\\
\caption{Mona Lisa, unsupervised genetic algorithm, fittest genome, 50,000 generations}
\label{evolutionfig}
\end{center}
\end{figure*}
...
\begin{figure*}[!htb]
\begin{center}
\includegraphics[width=2in]{../TEMP/P-2.png}\qquad
\includegraphics[width=2in]{../TEMP/P-10.png}\qquad
\includegraphics[width=2in]{../TEMP/P-50.png}\\
\includegraphics[width=2in]{../TEMP/P-107.png}\qquad
\includegraphics[width=2in]{../TEMP/P-203.png}\qquad
\includegraphics[width=2in]{../TEMP/P-352.png}\\
\includegraphics[width=2in]{../TEMP/P-964.png}\qquad
\includegraphics[width=2in]{../TEMP/P-1363.png}\qquad
\includegraphics[width=2in]{../TEMP/P-5922.png}\\
\includegraphics[width=2in]{../TEMP/P-12426.png}\qquad
\includegraphics[width=2in]{../TEMP/P-30034.png}\qquad
\includegraphics[width=2in]{../TEMP/P-44778.png}\\

\caption{Picasso, unsupervised genetic algorithm, fittest genome, 50,000 generations}
\label{evolutionfig}
\end{center}
\end{figure*}
...
\section{Discussion}
Contrary to our expectations, the more forced learning that we attempted to insert into the process, the worse the image quality became. When sticking to simpler methods, our processed images generally appeared to be able to be able to adapt to the image much more concretely in exchange for having an intially slow start in mutating. In comparison, our supervised methods mutated constantly initially but were unable to restructure itself to the finer details.

However, we also note that the unsupervised method would have difficulty in refining the details of the image. We believe that a more robust way would be to then add a supervised algorithm after the unsupervised portion in order to fine tune the details. As a result, once the base image had been formed as we can see is happening for our unsupervised algorithm, the finer details can be fleched out. There is unfortunately no cross point at which we can determine when the unsupervised portion should terminate but based on our experiments, 50,000 iterations appears to be the limitation or threshold at which the unsupervised algorithm is unable to refine the image further without an excessive amount of computation.

\section{Conclusion}
For our project, we implemented an image painter that evolves images toward a target with a genetic algorithm. Although our results are encouraging in that the overall structure of the brush strokes is a reasonable representation of the original image, many improvements could be made.  First, we confirmed our original hypothesis of allowing for arbitrary spline curves as brush strokes greatly increases the size of the problem space. Thus, with the additional computation overhead of evaluating splines, many more generations are required for images to converge to a better optimum. Given more time, we would like to run our simulations for a greater number of generations; note that Alsing's Mona Lisa was run for over 900,000 generations, while our maximum iteration was 50,000, given the time constraints. Another approach we have explored to better handle the large problem space was to provide supervision in the learning, such as modifying population parameters based on the generation, the probability and distribution of mutations, and combining different methods of mutation. Finally, the rendering aggregator library we used to paint the strokes in Python greatly limits the effects and weighting that can be applied to each brush stroke, and in the future we would like to fulfill our original goal of being able to render the brush strokes with additional effects and weighting. Images of our supervised genetic algorithm are shown in the Appendix.

\section{Acknowledgements}
Kevin Zhang and Andrew Wang worked equally on the project, splitting strategies to test. Initially, both pair programmed to get an unsupervised version to work. After the unsupervised version, both worked independently to come up with supervised strategies to beat the unsupervised model. The paper was also cowritten equally.
...
\begin{figure*}[!htb]
\begin{center}
\includegraphics[width=2in]{../TEMP2/M-102.png}\qquad
\includegraphics[width=2in]{../TEMP2/M-156.png}\qquad
\includegraphics[width=2in]{../TEMP2/M-288.png}\\
\includegraphics[width=2in]{../TEMP2/M-427.png}\qquad
\includegraphics[width=2in]{../TEMP2/M-515.png}\qquad
\includegraphics[width=2in]{../TEMP2/M-1161.png}\\
\includegraphics[width=2in]{../TEMP2/M-1813.png}\qquad
\includegraphics[width=2in]{../TEMP2/M-3178.png}\qquad
\includegraphics[width=2in]{../TEMP2/M-4578.png}\\
\includegraphics[width=2in]{../TEMP2/M-14630.png}\qquad
\includegraphics[width=2in]{../TEMP2/M-35131.png}\qquad
\includegraphics[width=2in]{../TEMP2/M-36522.png}\\
\caption{Mona Lisa, supervised genetic algorithm, fittest genome, 50,000 generations.}
\label{evolutionfig}
\end{center}
\end{figure*}
...
\begin{figure*}[!htb]
\begin{center}
\includegraphics[width=2in]{../TEMP2/P-126.png}\qquad
\includegraphics[width=2in]{../TEMP2/P-199.png}\qquad
\includegraphics[width=2in]{../TEMP2/P-629.png}\\
\includegraphics[width=2in]{../TEMP2/P-1416.png}\qquad
\includegraphics[width=2in]{../TEMP2/P-1889.png}\qquad
\includegraphics[width=2in]{../TEMP2/P-3921.png}\\
\includegraphics[width=2in]{../TEMP2/P-4321.png}\qquad
\includegraphics[width=2in]{../TEMP2/P-6558.png}\qquad
\includegraphics[width=2in]{../TEMP2/P-7792.png}\\
\includegraphics[width=2in]{../TEMP2/P-8391.png}\qquad
\includegraphics[width=2in]{../TEMP2/P-14741.png}\qquad
\includegraphics[width=2in]{../TEMP2/P-49320.png}\\
\caption{Picasso, supervised genetic algorithm, fittest genome, 50,000 generations.}
\label{evolutionfig}
\end{center}
\end{figure*}
